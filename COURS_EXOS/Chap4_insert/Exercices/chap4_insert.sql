...
SET `birth_day` = '1977-01-04 00:00:00',
 ...
 -- créer une clé primaire sur la table new_pilots

-- création d'une clé primaire sur une colonne existante (attention à l'unicité et aux valeurs en général dans la colonne )
ALTER TABLE `new_pilots` ADD CONSTRAINT `pk_pilots_company` PRIMARY KEY (`certificate`);


-- mettre des valeurs dans une table à partir d'une autre attention à la structure des tables de départ et d'arrivé
